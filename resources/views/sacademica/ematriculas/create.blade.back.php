...
{!! Form::open(['method'=>'get','role'=>'search','route'=>'sacademica.matriculas.create']) !!}
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
                    ...
                    {!! Form::select('tipoMatricula', tmUnidad(), null, ['id'=>'tipoMatricula','class'=>'form-control']) !!}
                </div>
                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-12">
...
{!! Form::close() !!}
@if (isset($unidades))
    @include('sacademica.ematriculas.modal')    
@endif
...
@section('js')
    ...
    function modalAgregar(index){
        ...
        if((Number(tcreditos) + Number(creditos))>24){
            alert('no se pueden exceder los 24 creditos');
        }else{
            //sumamos los creditos
            tcreditos = Number(tcreditos) + Number(creditos);
            $("#creditos").val(tcreditos);
            var fila='<tr class="selected" id="filaa'+id+'"><td><input type="hidden" name="idUniDidactica[]" value="'+id+'">'+ciclo+'</td><td>'+tipo+'</td><td>'+unidad+'</td><td>'+creditos+'</td><td><input type="hidden" name="tipoMatricula[]" value="'+tmatricula+'">'+tmatricula+'</td><td>'+id+'</td><td><button type="button" class="btn btn-warning" onClick="eliminar('+id+')">X</button></td></tr>';
