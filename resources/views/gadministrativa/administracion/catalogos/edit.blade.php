...
{!! Form::model($catalogo, ['route'=>['gadministrativa.administracion.catalogos.update',$catalogo->id],'method'=>'PUT','id'=>'frm']) !!}
    <x-adminlte-card title="Datos del nuevo catálogo" theme="info" icon="fab fa-product-hunt" collapsible>
        <div class="row">
            ...
            <div class="col-sm-12 col-md-3">
                ...
                {!! Form::text('modelo', null, ['class'=>'form-control','required'=>true]) !!}
            </div>
            ...
            <div class="col-sm-12 col-md-3">
                ...
                <select name="marcas" id="marcas" class="form-control selectpicker" title="No hay selección" data-live-search = true data-size = 10 required>
                    @foreach ($marcas as $marca)
                        <option value="{{ $marca->id }}" @if($catalogo->marca->id == $marca->id) selected @endif>{{ $marca->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-sm-12 col-md-3">
                ...
                <select name="tipos" id="tipos" class="form-control d-inline selectpicker" title="No hay selección" data-live-search = true data-size = 10 required>
                    @foreach ($tipos as $tipo)
                        <option value="{{ $tipo->id }}" @if($catalogo->tipo->id == $tipo->id) selected @endif>{{ $tipo->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-sm-12 col-md-3">
                 ...
                 <select name="unidades" id="unidades" class="form-control d-inline selectpicker" title="No hay selección" data-live-search = true data-size = 10 required>
                    @foreach ($unidades as $unidade)
                        <option value="{{ $unidade->id }}" @if($catalogo->unidade->id == $unidade->id) selected @endif >{{ $unidade->nombre }}</option>
                    @endforeach
                 </select>
            ...
            <div class="col-sm-12 col-md-3">
                ...
                @if ($catalogo->serie == true)
                    <x-adminlte-input-switch name="serie" data-on-text="SI" data-off-text="NO" data-on-color="teal" checked/>    
                @else
                    <x-adminlte-input-switch name="serie" data-on-text="SI" data-off-text="NO" data-on-color="teal" unchecked/>
                @endif
            </div>
            <div class="col-sm-12 col-md-3">
                <label for="" class="mt-2">¿Es perecible?</label>
                @if ($catalogo->perecible == true)
                    <x-adminlte-input-switch name="perecible" data-on-text="SI" data-off-text="NO" data-on-color="teal" checked/>    
                @else
                    <x-adminlte-input-switch name="perecible" data-on-text="SI" data-off-text="NO" data-on-color="teal" unchecked/>
                @endif
            </div>
            <div class="col-sm-12 col-md-6">
                {!! Form::label('padre', 'Categoria Padre', ['class'=>'mt-3']) !!}
                <select name="padre" id="padre" class="form-control selectpicker" title="No hay selección" data-live-search="true" data-size="10">
                    @foreach ($catalogos as $cata)
                        <option value="{{ $cata->id }}" @if(isset($catalogo->padre_id)) @if($catalogo->padre_id == $cata->id) selected @endif @endif>{{ $cata->codigo }} {{ $cata->modelo }} {{ $cata->descripcion }} - {{ $cata->unidade->nombre }}</option>
                    @endforeach
                </select>
            </div>
        </div>
