...
<!-- Portada -->
@section('tipo','Equivalencia')
@section('programa')
    {{ $uasignada->unidad->old->modulo->carrera->nombreCarrera  }}
    <!-- $uasignada->unidad->old -->
@endsection
@section('modulo')
    {{ $uasignada->unidad->old->modulo->nombre }}
@endsection
@section('udidactica')
    {{ $uasignada->unidad->old->nombre }}
@endsection
@section('periodo')
...
@endsection
@section('creditos')
    {{ $uasignada->unidad->old->creditos }}
@endsection
@section('horas')
    {{ $uasignada->unidad->old->horas }}
@endsection
@section('docente')
    {{ $uasignada->user->name }}
@endsection
...
@section('acta_notas')
    @isset($uasignada->unidad->old->id)
        ...
        <tr>
            ...
            <td class="pl-2 pb-0 pt-0 border"><span class="text-uppercase">{{  $estudiante->apellido }},</span> <span class="text-capitalize">{{ strtolower($estudiante->nombre) }}</span></td>
            <td class="pl-2 pb-0 pt-0 border text-center">{{ $estudiante->periodo }}</td>
                @if ($estudiante->tipo == "Convalidacion" || $estudiante->licencia == "SI")
